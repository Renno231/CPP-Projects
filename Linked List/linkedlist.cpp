...
void LinkedList::printList(bool reversed){
    ...
    while(current){
        printout.append("Index: " + std::to_string(counter) + " | Values: ID = " + std::to_string(current->data.id) + "; DATA = " + (current->data.data) + "\n");
        counter++;
        current = current->next; 
    }
    if (reversed){
        std::stringstream ss(printout);
        std::string to;
        std::string buffer;
